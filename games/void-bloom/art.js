...
window.VoidArt=(()=>{
  const art={hero:null,earth:null,carrier:null,poses:[]};
  // Source atlas rectangles: five pilot poses, the wide carrier, and the Earth horizon.
  function crop(source,x,y,w,h){const c=document.createElement("canvas");c.width=w;c.height=h;c.getContext("2d").drawImage(source,x,y,w,h,0,0,w,h);return c;}
  ...
  function portrait(){const c=document.getElementById('heroPortrait');if(!c||!art.hero)return;const x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);x.imageSmoothingEnabled=false;x.drawImage(art.hero,60,12,180,358);}
  art.ready=loadKeyed('assets/defense-sheet.png',true).then(sheet=>{
    if(!sheet)return;
