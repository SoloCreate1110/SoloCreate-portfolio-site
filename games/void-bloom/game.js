...
function start(){runDifficulty=selectedDifficulty;runMods=difficultyModifiers(runDifficulty);branches=Array(6).fill(0);runRelics=[];weaponTotals=Array.from({length:6},()=>({damage:0,kills:0}));runLog=[];runHits=0;bossRewardPending=false;focusToggle=false;waveVariant='normal';lastKillSound=-1;regenClock=0;runStartedAt=new Date().toISOString();logSaved=false;enemyGrid.clear();orangeGrid.clear();orangeShots.length=0;unlockAudio();rerolls=1;banishes=1;banishMode=false;banned=new Set();skillLevels=skills.map(()=>0);state='playing';ship={x:240,y:630,hp:maxHp(),inv:2,hearts:1};wave=1;score=0;xp=0;level=1;need=6;time=0;waveTime=0;spawnClock=0;spawned=0;levels=Array(6).fill(0);levels[heroes[selectedHero].weapon]=1;profile.weapons.push(heroes[selectedHero].weapon);profile.weapons=[...new Set(profile.weapons)];saveProfile();cd=[0,0,0,0,0,0];enemies=[];bullets=[];hostile=[];hazards=[];gems=[];effects=[];particles=[];kills=0;transition=0;keys={};pointer=null;shieldCd=0;barrier=0;shake=0;flash=0;$('overlay').innerHTML='';$('overlay').className='overlay';$('pause').textContent='Ⅱ';$('bossHud').hidden=true;hud('sector','textContent',stages[0].name);ui();arsenal();toast('WAVE 01 — FIRST CONTACT');}
...
function enemy(x,y,type=0,boss=false){const baseHp=boss? (wave===12?9800:wave===4?1200:1500+wave*160):Math.round(4+wave*1.4+wave*wave*.18)+(type===2?5+wave:0);const hp=Math.max(1,Math.round((!boss&&wave>=9?Math.round(baseHp*1.45):baseHp)*runMods.hp));return {x,y,type,boss,hp,max:hp,bossWave:boss?wave:0,r:boss?47:16,v:rand(15,24)+wave*2,t:rand(0,6.28),fire:rand(2.5,5),hit:0,salvos:0};}
function wavePacing(){return {rows:wave===1?5:wave<=3?4:3+Math.floor(wave/3)+(wave>=9?1:0),count:Math.max(2,Math.round((wave===1?4:wave<=3?5:5+Math.min(2,Math.floor(wave/4)))*runMods.count)),interval:(wave===1?3.6:wave===2?3.5:wave===3?3.2:wave>=9?2.15:2.6)*(waveVariant==='swarm'?.75:1)/runMods.spawn};}
function spawn(){if(wave%4===0){if(!spawned){const boss=enemy(240,-70,3,true);enemies.push(boss);seeEnemy(boss);runLog.push({type:'boss-entry',wave,level,hp:ship.hp,weapons:levels.slice(),atSeconds:+time.toFixed(2)});spawned=1;hud('bossName','textContent',BOSS_NAMES[wave]);toast('WARNING — '+BOSS_NAMES[wave],'#ff94bd');$('bossHud').hidden=false;}return;}const {rows,count}=wavePacing();if(spawned>=rows)return;for(let i=0;i<count;i++){
  if(wave>=5&&spawned===1&&i===Math.floor(count/2))enemies.push(specialEnemy('warper'));
    ...
    if(wave<=3)e.hp=e.max=Math.max(1,Math.round((wave===1?3:wave===2?5:7+(type===2?2:0))*runMods.hp));
...
function hostileShot(shot){shot.vx*=runMods.speed;shot.vy*=runMods.speed;hostile.push(shot);}
...
function updateBoss(e,dt){
  ...
  if(stage===8&&busy){
    e.coverFire=(e.coverFire??.8)-dt*runMods.rate;
    ...
    e.coverFire=(e.coverFire??.7)-dt*runMods.rate;
