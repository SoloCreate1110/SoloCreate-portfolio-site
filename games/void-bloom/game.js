...
// Apply only a small part of the extra pressure before the player can upgrade.
const DIFFICULTY_RAMP=[0,.08,.16,.28,.38,.52,.62,.72,.8,.9,.96,1,1];
function difficultyScale(key,atWave=wave,mods=runMods){const value=mods[key];return value<=1?value:1+(value-1)*DIFFICULTY_RAMP[Math.max(1,Math.min(12,atWave))];}
