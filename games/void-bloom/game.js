...
const ENEMY_SHOTS={orange:{color:'#ff9a3c',core:'#4c240b'},red:{color:'#ff3c55',core:'#4c0a19'},purple:{color:'#be60ff',core:'#310c4c'}};
const canCancel=b=>(b.kind||'orange')==='orange';
const supports=[{name:'船体修復',desc:'HPを25回復する。',icon:'＋'},{name:'緊急バリア',desc:'4秒間、橙色の敵弾と接触ダメージを防ぐ。赤・紫は防げない。',icon:'◇'},{name:'弾幕消去',desc:'橙色の敵弾を消去。赤・紫は残る。スコア +1,500。',icon:'✳'}];
const weapons=[
...
function upgradeDetailsHtml(i,current){return '<span class="upgrade-details">'+upgradeDetails(i,current).map(row=>`<span class="upgrade-stat"><span>${row.label}</span><strong>${row.value}</strong></span>`).join('')+'</span>';}
let state='title',ship={x:240,y:630,hp:MAX_HP,inv:0},wave=1,score=0,xp=0,level=1,need=6,time=0,waveTime=0,spawnClock=0,spawned=0,levels=[1,0,0,0,0,0],cd=[0,0,0,0,0,0],enemies=[],bullets=[],hostile=[],hazards=[],particles=[],gems=[],effects=[],keys={},shieldCd=0,barrier=0,sound=false,audio=null,best=0,shake=0,flash=0,kills=0,choices=[],pointer=null,transition=0,toastTimer=0,visualTime=0;
...
function start(){unlockAudio();state='playing';ship={x:240,y:630,hp:MAX_HP,inv:2};wave=1;score=0;xp=0;level=1;need=6;time=0;waveTime=0;spawnClock=0;spawned=0;levels=[1,0,0,0,0,0];cd=[0,0,0,0,0,0];enemies=[];bullets=[];hostile=[];hazards=[];gems=[];effects=[];particles=[];kills=0;transition=0;keys={};pointer=null;shieldCd=0;barrier=0;shake=0;flash=0;$('overlay').innerHTML='';$('overlay').className='overlay';$('pause').textContent='Ⅱ';$('bossHud').hidden=true;ui();arsenal();toast('WAVE 01 — FIRST CONTACT');}
...
function enemy(x,y,type=0,boss=false){const hp=boss? (wave===12?6800:1500+wave*160):Math.round(4+wave*1.4+wave*wave*.18)+(type===2?5+wave:0);return {x,y,type,boss,hp,max:hp,r:boss?47:16,v:rand(15,24)+wave*2,t:rand(0,6.28),fire:rand(2.5,5),hit:0};}
function spawn(){if(wave%4===0){if(!spawned){enemies.push(enemy(240,-70,3,true));spawned=1;toast(wave===12?'FINAL WAVE — VOID CORE':'WARNING — DREADNOUGHT','#ff94bd');$('bossHud').hidden=false;}return;}const rows=3+Math.floor(wave/3);if(spawned>=rows)return;const count=5+Math.min(2,Math.floor(wave/4));for(let i=0;i<count;i++){
  if(wave>=2&&spawned===1&&i===Math.floor(count/2))enemies.push(specialEnemy('warper'));
  else if(wave>=3&&spawned===2&&i===Math.floor(count/2))enemies.push(specialEnemy('sniper'));
  else if((spawned===2&&i===count-1)||(wave>=2&&spawned===0&&i===0))enemies.push(specialEnemy('sentry',spawned===0?'left':wave%2?'right':'top'));
  else if(wave>=6&&spawned===rows-1&&i===1)enemies.push(specialEnemy('warper'));
  else enemies.push(enemy(65+i*(350/(count-1)),-35-spawned%2*14,Math.floor(rand(0,Math.min(3,1+wave/3)))));
}spawned++;}
...
function hurt(amount=12+wave,kind='orange'){if(ship.inv>0||(barrier>0&&kind==='orange')||state!=='playing')return;ship.hp=Math.max(0,ship.hp-amount);ship.inv=1.1;shake=reduced?0:12;flash=.2;burst(ship.x,ship.y,'#ff698e',25);tone(110,.3,'sawtooth',.06,-70);ui();if(ship.hp<=0)finish(false);}
...
for(const e of enemies){if(e.hp<=0)continue;e.t+=dt;e.hit-=dt;e.fire-=dt;if(e.role){updateSpecial(e,dt);continue;}if(e.boss){e.y=Math.min(135,e.y+dt*65);e.x=240+Math.sin(e.t*.65)*145;if(e.fire<=0&&e.y>90){const n=wave===12?17:11;for(let j=0;j<n;j++){const a=Math.PI/2+(j-(n-1)/2)*.16+Math.sin(e.t)*.2;hostile.push({x:e.x,y:e.y+30,vx:Math.cos(a)*(150+wave*4),vy:Math.sin(a)*(150+wave*4),r:6,dmg:18+wave,kind:j%5===0?'purple':j%3===0?'red':'orange'});}e.fire=wave===12?.7:1;}$('bossLife').style.width=Math.max(0,e.hp/e.max*100)+'%';}else{e.y+=e.v*dt;e.x+=Math.cos(e.t*1.4)*dt*(e.type===1?24:9);if(e.fire<=0&&e.y>35&&e.y<510&&time>7){const aim=Math.atan2(ship.y-e.y,ship.x-e.x);const count=e.type===2&&wave>=3?3:1;for(let j=0;j<count;j++){const a=aim+(j-(count-1)/2)*.18;hostile.push({x:e.x,y:e.y+15,vx:Math.cos(a)*(105+wave*5),vy:Math.sin(a)*(105+wave*5),r:5,dmg:10+wave+e.type*2,kind:['orange','red','purple'][e.type]});}e.fire=rand(2.5,4.5)/(1+wave*.055);}if(e.y>H-10){e.hp=0;hurt(15+wave);}}if(Math.hypot(e.x-ship.x,e.y-ship.y)<e.r+13){hurt(22+wave);if(!e.boss)damage(e,999,'#ff698e');}}
...
window.voidBloom={snapshot:()=>({state,wave,score,level,xp,need,kills,hp:ship.hp,time,weapons:[...levels],enemies:enemies.length,bullets:bullets.length,hostile:hostile.length,hazards:hazards.length,ship:{x:ship.x,y:ship.y},auto:true,focus:!!keys.Space,maxHp:MAX_HP})};
})();
