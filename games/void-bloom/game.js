...
function showBossReward(){state='reward';const pool=relics.filter(r=>!hasRelic(r.id));rewardChoices=[];while(rewardChoices.length<3&&pool.length)rewardChoices.push(pool.splice(Math.floor(Math.random()*pool.length),1)[0]);openPanel('ボス撃破報酬','<p>特別コアを1つ選択。効果はこの出撃中持続。</p>'+rewardChoices.map((r,i)=>`<button class="pilot-card" id="reward${i}"><b>${r.name}</b><span>${r.desc}</span></button>`).join(''));rewardChoices.forEach((_,i)=>bindMenu('reward'+i,()=>pickReward(i)));}
