...
function draw(){ctx.clearRect(0,0,W,H);ctx.fillStyle='#070c17';ctx.fillRect(0,0,W,H);ctx.fillStyle=arenaGradient;ctx.fillRect(0,0,W,H);ctx.save();if(shake&&!reduced)ctx.translate(rand(-shake,shake),rand(-shake,shake));ctx.shadowBlur=0;for(const s of stars){ctx.globalAlpha=.25+s.z*.6;ctx.fillStyle=s.z>.8?'#a5d9d0':'#657d9f';ctx.fillRect(s.x,s.y,s.s,s.s);}ctx.globalAlpha=1;window.VoidArt?.background(ctx,visualTime);window.VoidArt?.mothership(ctx,visualTime);ctx.strokeStyle='#5ab9e025';ctx.lineWidth=1;ctx.setLineDash([4,7]);ctx.beginPath();ctx.moveTo(0,H*.7);ctx.lineTo(W,H*.7);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=zoneGradient;ctx.fillRect(0,H*.7,W,H*.3);
if(state==='title'){for(let row=0;row<3;row++)for(let n=0;n<7;n++)drawEnemy({x:57+n*61+Math.sin(visualTime)*6,y:92+row*54,type:row,hit:0,t:visualTime});drawShip(240,592,true);}else{for(const g of gems){if(window.VoidRender){window.VoidRender.gem(ctx,g);continue;}ctx.save();glow('#ffe65b',6);ctx.beginPath();ctx.moveTo(g.x,g.y-6);ctx.lineTo(g.x+4,g.y);ctx.lineTo(g.x,g.y+6);ctx.lineTo(g.x-4,g.y);ctx.closePath();ctx.strokeStyle='#302708';ctx.lineWidth=2;ctx.stroke();ctx.fill();ctx.restore();}for(const e of enemies)drawEnemy(e);ctx.shadowBlur=0;for(const b of bullets){if(window.VoidRender){window.VoidRender.bullet(ctx,b);continue;}glow(b.c,10);ctx.lineWidth=b.r*1.4;ctx.beginPath();ctx.moveTo(b.x,b.y);ctx.lineTo(b.x-b.vx*.018,b.y-b.vy*.018);ctx.stroke();ctx.fillStyle='#fff';ctx.fillRect(b.x-1,b.y-3,2,4);}drawShip(ship.x,ship.y);if(levels[5]){const n=weaponStats(5,levels[5]).count;for(let j=0;j<n;j++){const a=time*1.7+j*Math.PI*2/n,x=ship.x+Math.cos(a)*44,y=ship.y+Math.sin(a)*25;glow('#f1e890',12);ctx.save();ctx.translate(x,y);ctx.rotate(Math.PI/4);ctx.strokeRect(-5,-5,10,10);ctx.restore();}if(levels[5]===5){ctx.globalAlpha=.25;ctx.beginPath();ctx.ellipse(ship.x,ship.y,48,32,0,0,7);ctx.stroke();ctx.globalAlpha=1;}}}
