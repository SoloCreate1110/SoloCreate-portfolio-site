...
function balanceRecord(){return{version:30,startedAt:runStartedAt,hero:heroes[selectedHero].name,state,seconds:+time.toFixed(2),wave,level,hits:runHits,score,weaponTotals:weaponTotals.map((t,i)=>({name:weapons[i].name,level:levels[i],branch:branches[i],damage:Math.round(t.damage),kills:t.kills})),relics:runRelics.slice(),waves:runLog.slice()};}
