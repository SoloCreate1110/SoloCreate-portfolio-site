...
function ui(){ $('revive').textContent=ship.hearts?'♥ 1':'♡ 0';$('revive').classList.toggle('spent',!ship.hearts);$('revive').setAttribute('aria-label',ship.hearts?'復活ハート：残り1回':'復活ハート：残り0回'); $('wave').innerHTML=String(wave).padStart(2,'0')+' <em>/ 12</em>';$('score').textContent=String(score).padStart(6,'0');$('health').textContent=Math.ceil(ship.hp)+' / '+maxHp();$('hpFill').style.width=(ship.hp/maxHp()*100)+'%';$('hpFill').classList.toggle('low',ship.hp<=30);$('focus').classList.toggle('active',!!keys.Space);$('focus').setAttribute('aria-pressed',!!keys.Space);$('level').textContent='LV. '+String(level).padStart(2,'0');$('xp').style.width=Math.min(100,xp/need*100)+'%';$('xpText').textContent=Math.floor(xp)+' / '+need+' XP';$('best').textContent=String(best).padStart(6,'0');}
function arsenal(){ $('skills').innerHTML=skills.map((k,i)=>skillLevels[i]?`<span>${k.icon} ${k.name} <b>L${skillLevels[i]}</b></span>`:'').join('')||'<span class="empty-skills">レベルアップで能力を追加</span>'; $('weaponCount').textContent=String(levels.filter(Boolean).length).padStart(2,'0')+' / 03';$('arsenal').innerHTML=weapons.map((w,i)=>`<div class="weapon ${levels[i]?'':'locked'}" style="--c:${w.color}"><span class="weapon-icon">${w.icon}</span><div class="weapon-info"><b>${levels[i]===5?w.evo:w.name}</b><small>${levels[i]===5?'EVOLUTION':!levels[i]&&levels.filter(Boolean).length>=MAX_WEAPONS?'装備枠がいっぱい':w.en}</small><div class="pips">${[1,2,3,4,5].map(n=>`<i class="${levels[i]>=n?'on':''}"></i>`).join('')}</div></div><span class="weapon-level">${levels[i]===5?'✦':levels[i]?'L'+levels[i]:'—'}</span></div>`).join('');}
function title(){state='title';$('overlay').className='overlay title-screen';$('overlay').innerHTML='<span class="badge">EARTH · LAST DEFENSE</span><h2>VOID<br><span>BLOOM</span></h2><p>地球を背に、最後の防衛線へ。<br>母艦とつながる命の管が、帰還への道。</p><div class="mission-stamp">EARTH DEFENSE INITIATIVE</div><p>移動は WASD / 矢印キー / ドラッグ<br>常時オート射撃。Space長押しで低速移動。</p><button class="start" id="start">出撃する　↗</button><span class="subtext">TETHER ONLINE · HUMANITY’S LAST HOPE</span>';$('start').onclick=start;}
function start(){unlockAudio();rerolls=1;banishes=1;banishMode=false;banned=new Set();skillLevels=skills.map(()=>0);state='playing';ship={x:240,y:630,hp:MAX_HP,inv:2,hearts:1};wave=1;score=0;xp=0;level=1;need=6;time=0;waveTime=0;spawnClock=0;spawned=0;levels=[1,0,0,0,0,0];cd=[0,0,0,0,0,0];enemies=[];bullets=[];hostile=[];hazards=[];gems=[];effects=[];particles=[];kills=0;transition=0;keys={};pointer=null;shieldCd=0;barrier=0;shake=0;flash=0;$('overlay').innerHTML='';$('overlay').className='overlay';$('pause').textContent='Ⅱ';$('bossHud').hidden=true;ui();arsenal();toast('WAVE 01 — FIRST CONTACT');}
...
function enemy(x,y,type=0,boss=false){const hp=boss? (wave===12?6800:wave===4?1200:1500+wave*160):Math.round(4+wave*1.4+wave*wave*.18)+(type===2?5+wave:0);return {x,y,type,boss,hp,max:hp,r:boss?47:16,v:rand(15,24)+wave*2,t:rand(0,6.28),fire:rand(2.5,5),hit:0,salvos:0};}
...
function spawn(){if(wave%4===0){if(!spawned){enemies.push(enemy(240,-70,3,true));spawned=1;toast(wave===12?'FINAL WAVE — VOID CORE':'WARNING — DREADNOUGHT','#ff94bd');$('bossHud').hidden=false;}return;}const {rows,count}=wavePacing();if(spawned>=rows)return;for(let i=0;i<count;i++){
  if(wave>=5&&spawned===1&&i===Math.floor(count/2))enemies.push(specialEnemy('warper'));
    ...
    const shooter=purpleCaster|| (wave===1?false:wave===2?i===count-1:wave===3?i%2===1:(i+spawned)%3!==0);
...
if(state==='title'){for(let row=0;row<3;row++)for(let n=0;n<7;n++)drawEnemy({x:57+n*61+Math.sin(visualTime)*6,y:92+row*54,type:row,hit:0,t:visualTime});drawShip(240,592,true);}else{for(const g of gems){ctx.save();glow('#ffe65b',6);ctx.beginPath();ctx.arc(g.x,g.y,3.5,0,Math.PI*2);ctx.strokeStyle='#302708';ctx.lineWidth=2;ctx.stroke();ctx.fill();ctx.restore();}for(const e of enemies)drawEnemy(e);for(const b of bullets){glow(b.c,10);ctx.lineWidth=b.r*1.4;ctx.beginPath();ctx.moveTo(b.x,b.y);ctx.lineTo(b.x-b.vx*.018,b.y-b.vy*.018);ctx.stroke();ctx.fillStyle='#fff';ctx.fillRect(b.x-1,b.y-3,2,4);}drawShip(ship.x,ship.y);if(levels[5]){const n=weaponStats(5,levels[5]).count;for(let j=0;j<n;j++){const a=time*1.7+j*Math.PI*2/n,x=ship.x+Math.cos(a)*44,y=ship.y+Math.sin(a)*25;glow('#f1e890',12);ctx.save();ctx.translate(x,y);ctx.rotate(Math.PI/4);ctx.strokeRect(-5,-5,10,10);ctx.restore();}if(levels[5]===5){ctx.globalAlpha=.25;ctx.beginPath();ctx.ellipse(ship.x,ship.y,48,32,0,0,7);ctx.stroke();ctx.globalAlpha=1;}}}
